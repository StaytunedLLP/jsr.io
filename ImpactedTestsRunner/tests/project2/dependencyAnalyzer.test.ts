...
describe("ImpactedTestAnalyzer - Project 2", () => {
  ...
  it("should get all local dependencies", async () => {
    const dependenciesResult = await analyzerProject2.getAllLocalDependencies();
  ...
  it("should handle errors when getting dependents of a file", async () => {
    const faultyAnalyzer = new ImpactedTestAnalyzer(["nonexistent"]);
    const getDependentsOf = faultyAnalyzer.getDependentsOf.bind(faultyAnalyzer);
